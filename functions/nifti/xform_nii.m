...
function [hdr, orient] = change_hdr(hdr, tolerance, preferredForm)
   ...
   if space_unit ~= 1
      ...
      %hdr.dime.xyzt_units = char(bitset(double(hdr.dime.xyzt_units),2,1)); %original
      hdr.dime.xyzt_units = char(bitset(double(hdr.dime.xyzt_units),2,1)); %edited on 04/02/2013 by taneli
      
      %hdr.dime.xyzt_units = char(bitset(hdr.dime.xyzt_units,3,0)); %original
      hdr.dime.xyzt_units = char(bitset(double(hdr.dime.xyzt_units),3,0)); %edited on 04/02/2013 by taneli
   end
