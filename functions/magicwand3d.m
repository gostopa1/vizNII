...
if ndims(im) ~=4
    error('Input 3D image must be in RGB format');
end
...
for idx = 1:length(ylist)
    ref_r = double(im(ylist(idx), xlist(idx), zlist(idx), 1));
    ref_g = double(im(ylist(idx), xlist(idx), zlist(idx), 2));
    ref_b = double(im(ylist(idx), xlist(idx), zlist(idx), 3));
